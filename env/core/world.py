...
class World(object):
    def __init__(self, size, custom, start_position, start_direction_index, food_position):
        ...
        self.world[0:size[0] - 1, 0] = self.WALL
        self.world[0:size[0] - 1, size[1] - 1] = self.WALL
        self.world[0, 0:size[1] - 1] = self.WALL
        self.world[size[0] - 1, 0:size[1]] = self.WALL
    ...
    def init_food(self):
        ...
        self.current_available_food_positions = set.difference(self.available_food_positions, set(snake.blocks))
        if not self.custom:
            # Choose a random position from available now
            chosen_position = random.sample(self.current_available_food_positions, 1)
            chosen_position = chosen_position[0]
        else:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if new_snake_head[0] not in range(1, self.size[0]-1) or new_snake_head[1] not in range(1, self.size[1]-1):
            ...
            if new_snake_head == np.where(self.world == self.FOOD):
                # Remove old food
                self.world[np.where(self.world == self.FOOD)] = 0
                # Add tail again
